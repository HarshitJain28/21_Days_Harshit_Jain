...
class_labels = ['Cristiano Ronaldo','Zlatan Ibrahimovic','Lionel Messi','Mbappe','Neymar']
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                result = 'You Look Like : ' + predicting('.'+str(img_obj.image.url))
                status = 'Match'
            except:
                result = "Input Image Can not be processed please change the image"
                status = "No Match"
            return render(request, 'predict/index.html', {'form': form, 'img_obj': img_obj, 'result': result, 'status':status})
    else:
        form = ImageForm()
